...
| All tags have parents | {% assign failures = "" | split: "" %}{% for tag in site.tags %}{% assign parent = tag.parents[0] %}{% unless parent %}{% assign failures = failures | push: tag.name %}{% endunless %}{% endfor %}{% if failures.size == 0 %}Pass ✅{% else %}FAIL ❌{% endif %}  |  {% if failures.size > 0 %}These tags have no `parents`: `{{ failures | join: " " }}`{% else %}No orphans!{% endif %} |
